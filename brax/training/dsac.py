...
"""Distributional Soft Actor-Critic training.

See: https://arxiv.org/abs/2004.14547
"""
...
ComputeReward = Callable[
    [RewarderState, Transition, PRNGKey], Tuple[RewarderState, jnp.ndarray]
]
Rewarder = Tuple[RewarderInit, ComputeReward]

RiskMode = Literal["none", "cvar", "wang"]
...
class TrainingState(flax.struct.PyTreeNode):
...
def risk_distortion(
    quantiles: jnp.ndarray, risk_mode: RiskMode, risk_level: float,
) -> jnp.ndarray:
    if risk_mode == "none":
        return quantiles
    elif risk_mode == "cvar":
        return quantiles * risk_level
    elif risk_mode == "wang":
        return jscipy.stats.norm.cdf(jscipy.stats.norm.icdf(quantiles) + risk_level)
    else:
        raise ValueError(
            f"Received unknown risk mode {risk_mode} and risk level {risk_level}"
        )


def make_dsac_networks(
    param_size: int,
    obs_size: int,
    action_size: int,
    ...
    class QModule(linen.Module):
        ...
        @linen.compact
        def __call__(
            self, obs: jnp.ndarray, actions: jnp.ndarray, quantiles: jnp.ndarray
        ):
            hidden = jnp.concatenate([obs, actions], axis=-1)
            quantiles_transformed = risk_distortion(
                quantiles, risk_mode=risk_mode, risk_level=risk_level
            )
            res = []
            for _ in range(self.n_critics):
                quantile_samples = networks.QuantileMLP(
                    layer_sizes=hidden_layer_sizes + (1,),
                    activation=linen.relu,
                    kernel_init=jax.nn.initializers.lecun_uniform(),
                )(hidden, quantiles_transformed)
                res.append(quantile_samples)
    ...
    dummy_quantiles = jnp.expand_dims(
        jnp.arange(0, num_quantiles) / float(num_quantiles), -2
    )
    ...
    value = networks.FeedForwardModel(
        init=lambda key: q_module.init(key, dummy_obs, dummy_action, dummy_quantiles),
...
def train(
    ...
    first_state = jax.tree_multimap(lambda *args: jnp.stack(args), *tmp_env_states)
    ...
    policy_model, value_model = make_dsac_networks(
    ...
    q_optimizer = normalization.bcast_local_devices(q_optimizer, local_devices_to_use)
    ...
    def alpha_loss(
        alpha: jnp.ndarray, policy_params: Params, transitions: Transition, key: PRNGKey
    ) -> jnp.ndarray:
    ...
    def quantile_regression_error(q, target_q, dones, quantiles, kappa):
        print(q.shape, target_q.shape, quantiles.shape)
        q_error = jnp.expand_dims(target_q, axis=-2) - jnp.expand_dims(q, axis=-1)
        print(q_error.shape)
        q_error *= jnp.expand_dims(dones, axis=-1)
        L = huber_loss(q_error, kappa)
        rho = (tau - (q_error < 0)).abs() * L / kappa
        return (rho * quantiles).sum(axis=-2).mean()

    def huber_loss(q_error, kappa):
        abs_q_error = q_error.abs()
        return jnp.where(
            abs_q_error <= kappa,
            0.5 * q_error ** 2,
            kappa * (abs_q_error - 0.5 * kappa),
        )
    ...
    def critic_loss(
        ...
        quantiles = jnp.random.uniform(key, (transitions.a_tm1.shape[0], num_quantiles))
        q_old_action = value_model.apply(
            q_params, transitions.o_tm1, transitions.a_tm1, quantiles
        )
        ...
        next_quantiles = jnp.random.uniform(key, (next_action.shape[0], num_quantiles))
        next_q_quantile_samples = value_model.apply(
            target_q_params, transitions.o_t, next_action, next_quantiles
        )
        next_q = next_q_quantile_samples.mean(axis=-3)
        next_v = jnp.min(next_q, axis=-1) - alpha * next_log_prob
        target_q = jax.lax.stop_gradient(
            transitions.r_t * reward_scaling + transitions.d_t * discounting * next_v
        )
        q_loss = quantile_regression_error(
            q_old_action, target_q, transitions.d_t, quantiles, kappa
        )
        return q_loss
    ...
    def actor_loss(
        ...
        quantiles = jnp.random.uniform(key, (action.shape[0], num_quantiles))
        q_quantile_samples_action = value_model.apply(
            q_params, transitions.o_tm1, action, quantiles
        )
        q_action = q_quantile_samples_action.mean(axis=-3)
    ...
    @jax.jit
    def update_step(
        state: TrainingState, transitions: jnp.ndarray,
        ...
        if compute_reward is not None:
            new_rewarder_state, rewards = compute_reward(
        ...
        alpha_loss, alpha_grads = alpha_grad(
            alpha, state.policy_optimizer.target, normalized_transitions, key_alpha
        )
        critic_loss, critic_grads = critic_grad(
            state.q_optimizer.target,
    ...
    def collect_data(training_state, state):
        ...
        logits = policy_model.apply(
            training_state.policy_optimizer.target, normalized_obs
        )
    ...
    def init_replay_buffer_fn(training_state, state, replay_buffer):
    ...
    init_replay_buffer = jax.pmap(init_replay_buffer_fn, axis_name="i")
    ...
    def run_one_dsac_epoch(carry, unused_t):
    ...
    def run_dsac_training_fn(training_state, state, replay_buffer):
    ...
    run_dsac_training = jax.pmap(run_dsac_training_fn, axis_name="i")
    ...
    while True:
        ...
        eval_sps = (
            episode_length * eval_first_state.core.reward.shape[0] / (time.time() - t)
        )
        metrics = dict(
            dict(
                {
        ...
        t = time.time()
        # optimization
        training_state, state, replay_buffer, training_metrics = run_dsac_training(
            training_state, state, replay_buffer
        )
    ...
    policy_params = jax.tree_map(lambda x: x[0], training_state.policy_optimizer.target)
...
def make_params_and_inference_fn(
    ...
    """Creates params and inference function for the DSAC agent."""
    (
        obs_normalizer_params,
        obs_normalizer_apply_fn,
    ) = normalization.make_data_and_apply_fn(normalize_observations)
    parametric_action_distribution = distribution.NormalTanhDistribution(
        event_size=action_size
    )
    policy_model, _ = make_dsac_networks(
        parametric_action_distribution.param_size,
        observation_size,
        action_size,
        risk_mode,
